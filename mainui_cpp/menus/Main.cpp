...
void CMenuMain::CMenuMainBanner::Draw()
{
	...
	EngFuncs::DrawLogo( "valve.mp4", 0, logoPosY, logoWidth, logoHeight );
}
...
void CMenuMain::_VidInit( void )
{
	...
	console.pos.x = 32;
	resumeGame.SetCoord( 32, 480);
	disconnect.SetCoord( 32, 520);
	//credits.SetCoord( 72, 280 );
	createGame.SetCoord( 32, 560 );

	configuration.SetCoord( 32, 600 );
	multiPlayer.SetCoord( 32, 640 );

	previews.SetCoord( 32,  680);

	// too short execute string - not a real command
	if( strlen( MenuStrings[IDS_MEDIA_PREVIEWURL] ) <= 3 )
		previews.SetGrayed( true );

	quit.SetCoord( 32, 720 );
...
void UI_Main_Precache( void )
{
	...
	EngFuncs::PrecacheLogo( "valve.mp4" );
}
