...
void UI_ApplyCustomColors( void )
{
	...
	while(( pfile = EngFuncs::COM_ParseFile( pfile, token )) != NULL )
	{
		...
		{
			UI_ParseColor( pfile, &uiColorWhite );
		}
	}

	int	r, g, b;

	UnpackRGB( r, g, b, uiColorWhite);
...
{
	if( !EngFuncs::FileExists( "media/back.json", TRUE ))
		return;

	char *afile = (char *)EngFuncs::COM_LoadFile( "media/back.json", NULL );
	char *pfile = afile;
	char token[1024];
...
int UI_VidInit( void )
{
	...
	if (ScreenWidth * 3 < ScreenHeight * 4)
	{
		...
		uiStatic.yOffset = (ScreenHeight / 2.0) / uiStatic.scaleX - static_cast<double>(768) / 2;
	}
	else
	{
