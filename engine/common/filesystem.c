...
//	const char* pstr3;
	...
	/*pstr2 = Q_strstr(in, "maps/zb3/");
	if (pstr)
		start = pstr2 + 5 - in;

	pstr3 = Q_strstr(in, "maps/tdm/");
	if (pstr)
		start = pstr3 + 5 - in;

	pstr4 = Q_strstr(in, "maps/classic/");
	if (pstr)
		start = pstr4 + 5 - in;

	pstr5 = Q_strstr(in, "maps/zb1/");
	if (pstr)
		start = pstr5 + 5 - in;

	pstr6 = Q_strstr(in, "maps/zsh/");
	if (pstr)
		start = pstr6 + 5 - in;

	pstr7 = Q_strstr(in, "maps/zbg/");
	if (pstr)
		start = pstr7 + 5 - in;

	pstr8 = Q_strstr(in, "maps/zbs/");
	if (pstr)
		start = pstr8 + 5 - in;
	*/
...
void FS_InitGameInfo( gameinfo_t *GameInfo, const char *gamedir )
{
	...
	Q_strncpy( GameInfo->basedir, "cstz", sizeof( GameInfo->basedir ));
	GameInfo->falldir[0] = 0;
	Q_strncpy( GameInfo->title, "Counter-Strike Techno:Zombies", sizeof( GameInfo->title ));
	...
	// .ico path
	Q_strncpy( GameInfo->iconpath, "cstz.ico", sizeof( GameInfo->iconpath ));
	...
	while(( pfile = COM_ParseFile( pfile, token )) != NULL )
	{
		...
		else if( !Q_stricmp( token, "type" ))
		{
			...
			if( !isGameInfo && !Q_stricmp( token, "singleplayer_only" ))
			{
				...
				if( !Q_stricmp( GameInfo->gamefolder, "cstz") )
		...
		else if( isGameInfo )
		{
			...
			else if( !Q_stricmp( token, "gamemode" ))
			{
				...
				// but for some reason it's marked as singleplayer_only.
				// Old WON version is fine.
				if( !Q_stricmp( token, "singleplayer_only" ) && Q_stricmp( GameInfo->gamefolder, "cstz") )
					GameInfo->gamemode = GAME_SINGLEPLAYER_ONLY;
...
static qboolean FS_ParseGameInfo( const char *gamedir, gameinfo_t *GameInfo )
{
	...
	Q_snprintf( filepath, sizeof( filepath ), "%s/cl_dlls/gamesettings.json", gamedir );
	Q_snprintf( liblist, sizeof( liblist ), "%s/cl_dlls/client.cst", gamedir );
	...
	if( host.rodir[0] )
	{
		...
		Q_snprintf( filepath_ro, sizeof( filepath_ro ), "%s/%s/gamesettings.json", host.rodir, gamedir );
		Q_snprintf( liblist_ro, sizeof( liblist_ro ), "%s/%s/client.cst", host.rodir, gamedir );
...
void FS_LoadGameInfo( const char *rootfolder )
{
	...
	if( !Sys_GetParmFromCmdLine( "-dll", SI.gamedll ) )
	{
		...
		Q_strncpy( SI.gamedll, "mp", sizeof( SI.gamedll ) );
#elif defined(_WIN32)
...
dll_user_t *FS_FindLibrary( const char *dllname, qboolean directpath )
{
	...
	if( !Q_strnicmp( dllname, "..\\cstz\\", 9 ) || !Q_strnicmp( dllname, "../cstz/", 9 ))
		start += 9;
...
wfile_t *W_Open( const char *filename, const char *mode )
{
	...
	const char	*comment = "Generated by CST:Z WadLib. ";
