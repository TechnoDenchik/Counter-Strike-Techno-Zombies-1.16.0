...
class CKnifeShelteraxe : public LinkWeaponTemplate < CKnifeShelteraxe,
	TGeneralData,
	BuildTGetItemInfoFromCSW < WEAPON_SHELTERAXE > ::template type,
	TWeaponIdleDefault,
	TDeployDefault
>
{
public:
	static constexpr const char* ClassName = "shelteraxe";
	static constexpr const char* V_Model = "models/v_shelteraxe.mdl";
	static constexpr const char* P_Model = "models/p_shelteraxe.mdl";
	...
	static constexpr const char* AnimExtension = "shelteraxe";
...
void CKnifeShelteraxe::PrimaryAttack(void)
{
	...
	SetThink(&CKnifeShelteraxe::DelayedPrimaryAttack);
...
void CKnifeShelteraxe::SecondaryAttack(void)
{
#ifndef CLIENT_DLL
	m_pPlayer->SetAnimation(PLAYER_ATTACK1);
#endif
	...
	SetThink(&CKnifeShelteraxe::DelayedSecondaryAttack);
	pev->nextthink = gpGlobals->time + 0.9;
...
void CKnifeShelteraxe::DelayedPrimaryAttack()
{
	BOOL fDidHit = FALSE;
	...
	case HIT_PLAYER:
	{
		...
		EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/tomahawk_slash1_hit.wav", VOL_NORM, ATTN_NORM, 0, 94);
	...
	{
		SendWeaponAnim(ANIM_SLASH_HIT, UseDecrement() != FALSE);
		EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/tomahawk_wall.wav", VOL_NORM, ATTN_NORM, 0, 94);
...
void CKnifeShelteraxe::DelayedSecondaryAttack()
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
	...
	case HIT_NONE:
	{
		EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/tomahawk_stab_miss.wav", VOL_NORM, ATTN_NORM, 0, 94);
		break;
	}
	case HIT_PLAYER:
	{
		EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/tomahawk_slash2_hit.wav", VOL_NORM, ATTN_NORM, 0, 94);
		m_pPlayer->m_iWeaponVolume = KNIFE_BODYHIT_VOLUME;
		fDidHit = TRUE;
		break;
	}
