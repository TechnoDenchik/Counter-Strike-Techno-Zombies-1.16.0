...
LINK_ENTITY_TO_CLASS(weapon_shelteraxe, CShelterAxe)
...
void CShelterAxe::Precache(void)
{
...
int CShelterAxe::GetItemInfo(ItemInfo *p)
{
...
void CShelterAxe::Holster(int skiplocal)
{
	m_pPlayer->m_flNextAttack = UTIL_WeaponTimeBase() + 0.5;
}

void CShelterAxe::WeaponAnimation(int iAnimation)
{
	int flags;
...
void CShelterAxe::PrimaryAttack(void)
{
	Swing(TRUE);
}

void CShelterAxe::SetPlayerShieldAnim(void)
{
...
void CShelterAxe::ResetPlayerShieldAnim(void)
{
	if (m_pPlayer->HasShield() == true)
	{
		if (m_iWeaponState & WPNSTATE_SHIELD_DRAWN)
...
bool CShelterAxe::ShieldSecondaryFire(int up_anim, int down_anim)
{
	if (m_pPlayer->HasShield() == false)
		return false;
...
void CShelterAxe::SecondaryAttack(void)
{
...
void CShelterAxe::Smack(void)
{
	DecalGunshot(&m_trHit, BULLET_PLAYER_CROWBAR, false, m_pPlayer->pev, false);
}

void CShelterAxe::SwingAgain(void)
{
	Swing(FALSE);
}

void CShelterAxe::WeaponIdle(void)
{
	ResetEmptySound();
...
int CShelterAxe::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
...
int CShelterAxe::Stab(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
