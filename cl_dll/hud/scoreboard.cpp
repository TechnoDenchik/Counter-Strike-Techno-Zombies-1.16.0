...
#include "imgui_utils.h"
	...
	int CHudScoreboard::DrawScoreboard(float fTime)
	{
		...
		if (gHUD.m_szServerName[0])
			...
			snprintf(szTitle, 80, "CST:Z Server (%s)", GetGameModeName());
		...
		if ((gHUD.m_iModRunning == MOD_ZB1 || gHUD.m_iModRunning == MOD_ZB2 || gHUD.m_iModRunning == MOD_ZB3 || gHUD.m_iModRunning == MOD_ZB4 || gHUD.m_iModRunning == MOD_ZE || gHUD.m_iModRunning == MOD_DM || gHUD.m_iModRunning == MOD_ZBS || gHUD.m_iModRunning == MOD_ZSH))
		{
			DrawScoreNew(false);
		}
		else
	...
	void CHudScoreboard::DrawScoreNew(bool bDivideTeam)
	{
		...
		const bool bIsZombieMode = gHUD.m_iModRunning == MOD_ZB1 || gHUD.m_iModRunning == MOD_ZB2 || gHUD.m_iModRunning == MOD_ZB3 || gHUD.m_iModRunning == MOD_ZB4;
		...
		for (int iColumn = 1; iColumn <= 2; iColumn++)
		{
			...
			if (bDivideTeam && iPlayerCount)
			{
				const bool bIsZombieMode = gHUD.m_iModRunning == MOD_ZB1 || gHUD.m_iModRunning == MOD_ZB2 || gHUD.m_iModRunning == MOD_ZB3 || gHUD.m_iModRunning == MOD_ZB4;
				if (iColumn == 2)
					sprintf(szBuf, "%s  (%d)", bIsZombieMode ? "Humans" : "CT", iPlayerCount);
				else
					sprintf(szBuf, "%s  (%d)", bIsZombieMode ? "Zombies" : "TR", iPlayerCount);
			...
			else if (!bDivideTeam && iPlayerCount)
			{
				if (iColumn == 2)
					...
					sprintf(szBuf, "%s  (%d)", bIsZombieMode ? "Zombies" : "TR", g_PlayerExtraInfo[SortedId[0]].frags);
			...
			if (iColumn == 1)
			{
				...
				for (int i = 0; i < 3; i++)
				{
					static constexpr const char* szLeaderNames[] = { "TechnoSoftware", "Nexon Corp", "Xash3D FWGS" };
			...
			else
			{
				...
				{
					std::tie(r, g, b, a) = std::make_tuple(255, 188, 0, 255);
					DrawUtils::DrawHudString(x + 35, y + iStartH - 212, (iStartW / 2) - 49 - (x + 35), "Spectator", r, g, b, flScale);
					gEngfuncs.pfnFillRGBA(x + 19, y + iStartH - 212, (iStartW / 2) - 49, 1, 188, 112, 0, 255);
				}
			}
		}
