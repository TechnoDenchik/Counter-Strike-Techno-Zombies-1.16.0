...
int CHudAmmo::Draw(float flTime)
{
	...
	if (m_pWeapon->iAmmoType > 0)
	{
		...
		if (pw->iClip >= 0)
		{
			...
			x = DrawUtils::DrawHudNumber2(x, y, iFlags | DHN_3DIGITS, 2, pw->iClip, r, g, b);
			...
			DrawUtils::UnpackRGB(r,g,b, RGB_WHITE);
			 int ammos = gWR.CountAmmo(pw->iAmmoType);
	         ...
	         DrawTexturedNumbersTopRightAligned(*winhm, m_rcTeamnumber, ammos, x4 + 70, y4 + 14, 1.20f);
			...
			x = DrawUtils::DrawHudNumber2(x, y, iFlags | DHN_3DIGITS, 2, ammos, r, g, b);
		...
		else
		{
			...
			x = DrawUtils::DrawHudNumber2(x, y, iFlags | DHN_3DIGITS,2 , gWR.CountAmmo(pw->iAmmoType), r, g, b);
		}
	...
	if (pw->iAmmo2Type > 0) 
	{
		...
		if ((pw->iAmmo2Type != 0) && (gWR.CountAmmo(pw->iAmmo2Type) > 0))
		{
			...
			x = ScreenWidth - 4 * AmmoWidth - iIconWidth;
			x = DrawUtils::DrawHudNumber2(x, y, iFlags|DHN_3DIGITS, 2, gWR.CountAmmo(pw->iAmmo2Type), r, g, b);
...
int CHudAmmo::DrawCrosshairEx(float flTime, int weaponid, int iBarSize, float flCrosshairDistance, bool bAdditive, int r, int g, int b, int a)
{
	...
	if (bDrawCircle)
	{
		float radius = (iBarSize / static_cast<float>(2)) + flCrosshairDistance;
#if 0
